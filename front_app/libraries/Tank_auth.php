...
class Tank_auth {
        ...
        if ((strlen($login) > 0) AND ( strlen($password) > 0)) {
            ...
            if (!is_null($user = $this->ci->users->$get_user_func($login))) { // login ok
                ...
                if ($hasher->CheckPassword($password, $user->password)) {  // password ok
                    ...
                    $m_type = $user->member_type;
                    $s_date = $user->premium_start_date;
                    $e_date = $user->premium_end_date;

                    if ($user->premium_end_date != NULL && strtotime($user->premium_end_date) < strtotime($current)) {
                        //echo $user->premium_end_date."=========>";exit;
                        $this->ci->users->downgrade_user($user->id);
                        $m_type = "general";
                        $s_date = NULL;
                        $e_date = NULL;
                    }
                    $this->ci->session->set_userdata(array(
                        ...
                        'email' => $user->email,
                        'member_type' => $m_type,
                        'premium_request' => $user->premium_request,
                        'status' => ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
                        'claim_status' => $user->claim_status,
                        'claimed_editor' => $user->claimed_editor,
                        'key' => $user->new_email_key,
                        'premium_start' => $s_date,
                        'premium_end' => $e_date,
                    ));
                    ...
                    } else if ($user->activated == 2) { // fail - not activated
     ...
     * @return	array
     */
    function create_user($username, $first_name, $last_name, $email, $password, $terms, $email_activation) {
        //echo $zip_code;
        //echo $state;
        //exit;
        if ((strlen($username) > 0) AND ! $this->ci->users->is_username_available($username)) {
            ...
            $slug = $this->ci->users->member_slug_generator(str_replace(" ", "-", strtolower($full_name)));
            $data = array(
                'slug' => $slug,
                'username' => $username,
            ...
            if (!is_null($res = $this->ci->users->create_user($data, !$email_activation))) {
                ...
                //$user_info->user_age='none';
                //$user_info->user_marital='none';
                //$user_info->user_gender='none';
                //$user_info->user_edu='none';
                //$user_info->user_income='none';
                //$user_info->user_zipcode=$zip_code;
                //$user_info->user_country=$state;
                //$user_info->user_residential='none';
                //$user_info->user_house_hold_1='none';
                //$user_info->user_house_hold_2='none';
    ...
    function admin_password_hash($new_pass) {
        $hasher = new PasswordHash(
        $this->ci->config->item('phpass_hash_strength', 'tank_auth'), $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
        $hashed_password = $hasher->HashPassword($new_pass);
    ...
    function my_password_match($password, $stored_password) {
        $hasher = new PasswordHash(
                        $this->ci->config->item('phpass_hash_strength', 'tank_auth'), $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
