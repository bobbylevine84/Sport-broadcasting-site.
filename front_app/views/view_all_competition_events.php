...
foreach($get_competition_events as $event){ 
 $meta_title = str_replace("%competition%",$event['competition_name'],$meta_title);	
              $meta_title = str_replace("%competitions%",$competition_names,$meta_title);

               $meta_description = str_replace("%competition%",$event['competition_name'],$meta_description);	
              

               $meta_keyword = str_replace("%competition%",$event['competition_name'],$meta_keyword);	
            

               $meta_article = str_replace("%competition%",$event['competition_name'],$meta_article);	
					...
					<span class="teamname">
						<a href="<?php echo base_url();?>competitions/<?php echo $event['competition_id'];?>/<?php if($event['category_name'] == "Soccer") { $event['category_name'] = "football"; } echo str_replace(' ', '-', $event['category_name']);?>/<?php echo strtolower(str_replace(' ', '-', $event['nation']));?>/<?php echo strtolower(str_replace(',','',str_replace(' ', '-', $event['competition_name'])));?>" style="background:none;color:red;  padding:5px 0; margin:0;" title="<?php echo ucwords($event['competition_name']); ?>">
							<span class="cont-nation cont-nation-<?php echo $this->session->userdata('session_color');?> ">
								...
								if(strlen($event['competition_name'])>45) {
									echo ucwords(substr($event['competition_name'],0,45)).'..';
								}
						...
						<div class="<?php if($event['away_team']) {?>col-sm-5 no-pad tablet-data1 dev-float-left<?php } else {?>col-sm-12 no-pad  tablet-data1 dev-float-left<?php } ?>">
							<div class="team-data1">
								...
								<span class="country-name">
									<a href="<?php echo base_url();?>teams/<?php echo ucwords($event['home_team_id']); ?>/<?php if($event['category_name'] == "Soccer") { $event['category_name'] = "football"; } echo str_replace(' ', '-', $event['category_name']);?>/<?php echo strtolower(str_replace(' ', '-', $event['nation']));?>/<?php echo strtolower(str_replace(' ', '-', $event['home_team']));?>">
									<?php
...
foreach($get_future_competition_events as $event){ ?>
			<tr class="white-color <?php echo $this->session->userdata('session_color');?>-color <?php echo $this->session->userdata('session_color');?>-hover">
				<td class="brdr-right">
					<div class="event-data ">
						<div class="text-left">
			...
			if($get_last_competition_events){
foreach($get_last_competition_events as $event){ ?>
			<tr class="white-color <?php echo $this->session->userdata('session_color');?>-color <?php echo $this->session->userdata('session_color');?>-hover">
				<td class="brdr-right">
					<div class="event-data ">
						<div class="text-left">
