...
<div class="row">
	...
	<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
		...
		<?php
			...
			$rss->join('rss_events','rss_events.competition_id=rss_competition.competition_id','left');
			$rss->where('rss_events.nation',$get_nation_events[0]['nation']);
			if($nation_sport_id){
				$this->db->where('rss_competition.sport_cat_id',$nation_sport_id);
				$this->db->join('rss_sport_category','rss_sport_category.id=rss_competition.sport_cat_id');
			}
		...
		<select name="my_competition" id="my_competition" >
			<option value="" <?php if($nation_sport){?> selected disabled <?php } ?> >All Competitions</option>
	...
	<table class="table datatable-<?php echo $this->session->userdata('session_color');?> table-striped table-striped-<?php echo $this->session->userdata('session_color');?> text-center" id="manage_events" border="0">
		<thead class="<?php echo $this->session->userdata('session_color'); ?>-thead">
			<tr><?php if($nation_sport){ } else { ?>
				...
				<th class="table-width5 text-center">Broadcast</th>
			</tr>
		</thead>
		<tbody id="show_competition" class="searchable">
			...
			<tr class="white-color <?php echo $this->session->userdata('session_color');?>-color <?php echo $this->session->userdata('session_color');?>-hover"><?php if($nation_sport) {  } else { ?>
				<td class="brdr-right">
