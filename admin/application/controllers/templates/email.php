...
class Email extends CI_Controller
{
	...
	public function update_process()
	{
		...
		$edit_template = array(
			...
			'email_type' => $this->input->post("type"),
		...
		$this->session->set_flashdata("ok_message", "Email Template is updated Successfully ");
		redirect(base_url().'templates/email/');
	...
	//Login Check
		...
		$this->breadcrumbcomponent->add('Maintenance Mode', base_url().'templates/email/manage_template');
		$data['breadcrum_data'] = $this->breadcrumbcomponent->output();
		...
		$query_mode = $this->db->query("
									SELECT * FROM
									kt_site_mode 
									 
								");
		$data['site_mode_status'] = $query_mode->result_array();
		$data['site_mode_status1'] = $data['site_mode_status'][0]['status'];
		$this->load->view('templates/email_templating',$data);
	...
	public function change_mode()
	{
		...
		$mode_status = $this->uri->segment(4);
		$contents = $this->input->post("mantances_content");
		$insert = array(
						'status'=>$mode_status,
						'contents'=>$contents
						);
		$updated = $this->db->update('kt_site_mode',$insert);
	}
