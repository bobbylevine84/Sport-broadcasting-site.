...
<form action="<?php echo base_url('settings/manage_setting/update_grp_ex');?>" method="post">
  <!-- Tab panes -->
  <div class="tab-content">
    <div role="tabpanel" class="tab-pane active" id="tab1">
		...
		<?php foreach($grp_exchange_settings as $data){
...
<input id="hidden" name="hidden" type="hidden" value="<?php echo $data['id']; ?>"/>
	<input id="ex_per" name="ex_per" value="<?PHP echo $data['ex_per'];?>" class="form-control" step="0.1" type="number" placeholder="30" />
	</div>
	</div>
    </div>
    <div class=" col-md-4">
	...
	<input id="ex_fix" name="ex_fix" value="<?PHP echo $data['ex_fix'];?>" class="form-control" type="number" step="0.1" placeholder="30" />
	</div>
	</div>
    </div>
    </div>
...
$('document').ready(function(){
    ...
    $("#select_group").change(function(){
		var data = $(this).val();
		//alert(data);
		$.post('<?php echo base_url() ?>settings/manage_setting/group_exchange', {'data': data}, function(data){
			...
			$.each( obj, function( key, val ) {
				...
				$('#ex_per').val(val.ex_per);
				$('#ex_fix').val(val.ex_fix);
				$('#hidden').val(val.id);
