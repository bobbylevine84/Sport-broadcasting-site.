        ...
        <div class="col-md-12">
          <div class="panel">
            ...
            <div class="panel-body alerts-panel">
              <form class="cmxform" id="add_new_slider_image_frm" method="POST" action="<?php echo SURL ?>product/manage_product/add_new_product_process/" enctype="multipart/form-data">
                  ...
                  <div id="cms_main_contents" class="tab-pane active">
                    ...
                    ?>
					 ...
					 <div class="form-group" style="padding-left: 0px;">
                                    ...
                                    <input id="brand" name="brand" value="<?php echo $product[0]['brand_name'];?>" type="text" class="form-control" style="width:50%;"/>
					  ...
					  <div class="form-group">
                        ...
                        <input type="text" id="product_type" name="product_type" value="<?php echo $product[0]['product_type'];?>" class="form-control" style="width:50%;">
					 ...
					 <div class="form-group col-md-12">
                            ...
                            <div class="col-md-12">
                                ...
                                <ul id="selected_tags" class="list-inline">
								
								<?php foreach($product as $row){?>
                                   <li class="tags_li" style="background-color: green; color: #fff; padding: 10px; margin: 0 5px 5px 0;"><?php echo $row['flavour_name'];?><img width="20px" height="20px" id="flavour_img" src="<?php echo SUR?>uploads/section/<?php echo $row['flavour_image'];?>" class="ui-state-default" alt=""/><a href="javascript:;" data-tag="<?php echo $row['flavour_name'];?>" class="glyphicon glyphicon-remove remove_tag_li" style="top: -10px; right: -10px; color: #fff;"></a><input type="hidden" name="selected_flavors[]" value=""><input type="hidden" name="selected_images[]" value="<?php echo $row['flavour_name'];?>"></li>
								<?php }?>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="form-group" align="right" style="margin-right:17px">
                    	<input class="submit btn btn-blue" type="submit" name="add_product" id="add_product" value="Add New Product" />
                    </div>
                </div>
...
<?php echo $INC_header_script_footer;?>
	...
	<script>
	  var tags_array = [];
        <?php if($repeat == "repeat") {
            if(!empty($existing_tags2)) { foreach($existing_tags2 as $tag) { ?>
            tags_array.push("<?php echo $tag; ?>");
        <?php } } }else if(!empty($product['flavour_name'])) { foreach($product['flavour_name'] as $row) { ?>
            tags_array.push("<?php echo $row; ?>");
        <?php } } ?>
        var publications_array = [];
        
        <?php if($repeat == "repeat") { if(!empty($selected_publication)) { foreach($selected_publication as $key => $publication) { if(is_numeric($publication)) { ?>
        ...
        <?php } } } }else if(!empty($publications)) { foreach($publications as $publication) { if(is_numeric($publication['publication'])) { ?>
            publications_array['<?php echo $publication['title']; ?>'] = '<?php echo $publication['publication']; ?>';
        <?php }else { ?>
            publications_array['<?php echo $publication['publication']; ?>'] = '<?php echo $publication['publication']; ?>';
        <?php } } } ?>
