                  ...
                  <table class="table table-striped table-bordered table-hover" id="manage_sponsor">
                    ...
                    <?php
							...
							for($i=0;$i<$stream_count;$i++){
								...
								$this->db->where('event_id_stream',$stream_arr[$i]['event_id_stream']);
								$this->db->where('stream_status','pending');
								//$this->db->group_by('event_id_stream');
								$count_streams = $this->db->get('kt_stream_raw')->num_rows;
								echo $count_streams;
								
					?>	
                            <tr>	<!-- Count rows -->
							<td style="vertical-align: middle;">
								<input name="raw_stream[]" type="checkbox" class="cb-element checkbox1" value="<?php echo $stream_arr[$i]['event_id_stream']; ?>"/>
							</td>
                                <td><span class="xedit"><?php echo ($i+1) ?></span></td>
                                <td class=""><?php echo $stream_arr[$i]['home_team'].' <b>VS</b> '.$stream_arr[$i]['away_team']; ?></td>
								<td class=""><?php echo $count_streams; ?>
								</td>
                               
                                <td class="">
									<a  href="<?php echo base_url();?>stream/manage_stream/update_stream_raw/<?php echo $stream_arr[$i]['event_id_stream']; ?>" class="btn <?php
