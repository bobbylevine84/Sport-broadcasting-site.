...
class Mod_email extends CI_Model {
	...
	public function get_all_trans()
	{
		...
		return $this->db->count_all('kt_sg_transactions');
	}
	public function get_all_trans_data(){
            $this->db->dbprefix = "";               
            $this->db->select("kt_sg_transactions.transaction_id as id, kt_sg_transactions.user_id_fk as user_id, kt_sg_transactions.amount as withdraw_amount, kt_sg_transactions.email as paypal_id, sg_front_users.first_name, sg_front_users.last_name");
            $this->db->join("sg_front_users", "sg_front_users.front_user_id = kt_sg_transactions.user_id_fk");
            $this->db->where('transaction_type', "withdraw");          
            $this->db->where('status', "pending");
            $this->db->order_by('created', "desc");
            $output = $this->db->get('kt_sg_transactions')->result();
            return $output;
